...
function displayItems() {
    ...
    const url = 'http://localhost:3000/api/ComSuon';
    xhr.open("GET", url);
    xhr.send();
    var ComSuonStore=[];
    xhr.onreadystatechange = (e) => {
        ...
        try{
            ComSuonStore = JSON.parse(res);
            // console.log(ComSuonStore);          
        }catch(err){
            console.log(err);
        }
       
        var ComSuon=document.getElementById('ComSuon')

        const ComSuonData=ComSuonStore;

        ComSuonData.map(item=>{
